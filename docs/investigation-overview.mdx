...
- **Direct investigation** — run `opensre investigate` from your terminal. In TTY sessions, running it with no input opens a guided picker (bundled demo alert, templates, custom file path, or paste JSON). The process runs and exits when the investigation completes.
...
In TTY terminals, running `opensre investigate` with no `--input`, `--input-json`, or piped stdin opens a guided picker with bundled sample/template options.
